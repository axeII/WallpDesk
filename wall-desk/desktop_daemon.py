...
class DesktopDaemon(Wallpaper):
    ...
    def check_desktop(self):
        while True:
            time.sleep(1.5)
            super().get_all_files()
            for supported_file_name in filter(
                lambda data: data.startswith("@"), self.all_files
            ):
                self.evaluate(self.lexer(supported_file_name))

    def run(self):
        Process(
            target=self.check_desktop, name="P_desktop_manager", daemon=True
        ).start()
        ...
        if token.command in list(parsed_data["paths"].keys()):
            subprocess.call(
                [
                    "mv",
                    "-n",
                    f"{HOME}/Desktop/{token.origin_name}",
                    os.path.join(
                        parsed_data["paths"][token.command], f"{token.file_name}"
                    ),
                ]
            )
            ...
            subprocess.call(
                [
                    ...
                    "-n",
                    f"{HOME}/Desktop/{token.origin_name}",
                    f"{token.command}/{token.file_name}",
                ]
            )
        else:
